...
tcgaSamples.analyse.sampleAnnotation(sample_annotation){
...
mirreti.twoConditions <- function(tpm_expr, mir_expr, sample_annotation, interactions, cluster.size, log = FALSE){
  ...
  for(c in unique(sample_annotation$condition)){
    expr_data <- mirreti.extractSamples.forCondition(tpm_expr = tpm_expr,
                                                     mir_expr = mir_expr,
...
# #' @param sample_annotation
# #' @param condition
# #' @return tpm_expr, mir_expr
mirreti.extractSamples.forCondition <- function(tpm_expr, mir_expr, sample_annotation, condition){
...
# #' @param log
# #' @return interactions
sponge.correlateData <- function(tpm_expr, mir_expr, interactions, condition = NULL, cluster.size, log = FALSE){
      start.time <- 0
      end.time <- 0
  ...
  candidates <- sponge.unlist.candidates(candidates)
