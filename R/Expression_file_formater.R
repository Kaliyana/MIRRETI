...
#'
#' @param expr_file Filepath to the expression file which should be subsetted.
#' @param sample_annot_file Filepath to the TCGA samples annotation file holding informations on TCGA sample IDs.
...
crossextend_expression_data <- function(mirna_expr, tpm_expr){

  # Build list of samples appearing in both expression datasets
  combined_sampleids <- c(row.names(mirna_expr), row.names(tpm_expr))
  unique_sampleids <- combined_sampleids[!duplicated(combined_sampleids)]
  if(!(all(row.names(mirna_expr) %in% unique_sampleids) & all(row.names(tpm_expr) %in% unique_sampleids))){
    cat("Sample IDs not combined properly. Please check again")
    quit
  }

  # Extend both expression datasets for respectively missing sample IDs
  extended_mirna_expr <- extend_expression_data(mirna_expr, unique_sampleids)
  extended_tpm_expr <- extend_expression_data(tpm_expr, unique_sampleids)
  return(list(extended_mirna_expr, extended_tpm_expr))
}
...
#-------------------------------------------------------------------------------------
#                         Extend single expression file
#-------------------------------------------------------------------------------------


# INCLUDE THIS ONE IN transpose_and_filter_expression_data_for_samples() TO MINIMIZE FUNCTION NUMBER
# SURELY RETURNS DIFFERENT NUMBER OF SAMPLES AS CURRENT WORKFLOW DOES NOT INCLUDE SAMPLE IDS MISSING
# IN BOTH EXPRESSION FILES
extend_expression_data <- function(expr_data, sample_list){

  # Spot sample IDs missing in the current expression dataset
  ...
  missing_expr_data <- as.data.frame(matrix(0,
                                            ...
                                            length(colnames(expr_data))))
  row.names(missing_expr_data) <- missing_sampleids
  colnames(missing_expr_data) <- colnames(expr_data)

  # Append matrix with missing sample IDs to the expression dataset
  extended_expr_data <- rbind(expr_data, missing_expr_data)
...
#expr_file <- "/nfs/home/students/evelyn/bachelor/data/TCGA_expression_data/pancanMiRs_EBadjOnProtocolPlatformWithoutRepsWithUnCorrectMiRs_08_04_16.xena"
...
#expr_subset <- process_expression_file(expr_file, sample_annot_file, "breast invasive carcinoma", "1")
